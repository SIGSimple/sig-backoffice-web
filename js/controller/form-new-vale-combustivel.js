...
app.controller('CadastroValeCombustivelCtrl', function($scope, $http, UserSrvc){
	$scope.colaborador = UserSrvc.getUserLogged();
	$scope.valeCombustivel = {
		beneficiario: {
			data: {
	...
	$scope.abreModal = function() {
		...
		$('#mytable').bootstrapTable({
			...
			onClickRow: function(row, $element) {
				$scope.valeCombustivel.beneficiario.data = row;
				$scope.valeCombustivel.beneficiario.label = $element.find("td").text();
				$scope.$apply();
				$('#mytable').bootstrapTable('destroy');
